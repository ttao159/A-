...
- **策略生成引擎**：输入标的范围、回测区间、风险偏好、生成数量与收益目标，启发式生成多个候选策略，使用公开 API 真实日线数据回测，输出多策略对比（指标表 + 叠加权益曲线）与推荐策略，支持一键保存为策略。
...
- 存储：SQLite
- 行情数据：策略生成引擎使用公开 HTTP API（腾讯 K 线前复权日线 + 新浪股票列表），禁用合成数据；扫描与账户功能沿用 akshare（断网降级合成数据）
...
| POST | `/api/generator/run` | 策略生成引擎：生成并对比多个候选策略（公开 API 真实日线回测） |

## 说明

- 行情数据优先使用 akshare 真实数据；当网络不可用或接口失败时，自动降级为带趋势的合成数据（按股票代码生成、可复现，价格围绕样本股真实价位基准波动），保证功能完整可演示。策略生成引擎除外：它仅使用腾讯/新浪公开 API 的真实日线数据，数据源不可用时直接返回错误，不使用合成数据。
