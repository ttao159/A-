...
- 存储：SQLite
- 行情数据：全系统统一使用公开 HTTP API（腾讯 K 线前复权日线 + 新浪股票列表），禁用合成数据，带内存 TTL 缓存与并发预取
...
- 全系统行情数据统一来自公开 HTTP 接口（腾讯 K 线前复权日线、新浪股票列表），真实行情、禁用合成数据；数据源不可用时接口返回明确错误。日线数据带内存缓存（10 分钟）与并发预取，全市场扫描约 2~3 分钟。
