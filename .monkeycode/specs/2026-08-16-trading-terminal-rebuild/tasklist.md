...
- [x] 9. 实盘安全风控与状态标注
  - [x] 9.1 实现实盘下单二次确认交互
    - 手动下单两阶段 prepare/confirm，下发前弹出确认框，实盘模式附加风险提示
  - [x] 9.2 实现下单金额限额与异常状态提示
    - 后端单笔/单日累计金额上限校验（live 模式生效），前端区分成交/拒绝并提示原因
  - [x] 9.3 界面标注实盘/模拟盘状态与风险提示
    - 顶部 badge 展示 broker_type，实盘模式显示风险横幅

- [x] 10. 检查点 - 最终验证与收尾
  - 后端 pytest 117 passed，前端 vue-tsc + vite build 通过，前后端代理联调各接口 200
